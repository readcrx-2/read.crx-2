...
exports.puts = mes => new Promise( function(resolve, reject) {
  console.log(mes);
...
const onRollupError = function(filename, type) {
  ...
  return function(e) {
    ...
    let mes = prefix + `${RED}${type}(${e.code})${RESET} '${CYAN}${filename}${RESET}': ${e.message}`;
    if (e.loc != null) { mes += "\n" + _spaceR + `${e.loc.file} L${e.loc.line}:${e.loc.column}`; }
    if (e.location != null) {
      const file = path.relative(process.cwd(), e.id);
      if (e.location != null) { mes += "\n" + _spaceR + `${file} L${e.location.first_line+1}:${e.location.first_column+1}-${e.location.last_column+1}`; }
    }
...
exports.onRollupWatch = filename => (function(e) {
  const prefix = `[${GRAY}${getTimeString()}${RESET}][${GRAY}Rollup.js${RESET}] `;
  switch (e.code) {
    case "START": case "END": return; break;
    case "BUNDLE_START": console.log(prefix + `Starting '${CYAN}${filename}${RESET}'`); break;
    case "BUNDLE_END": console.log(prefix + `Finished '${CYAN}${filename}${RESET}' in ${e.duration}ms`); break;
    case "FATAL": onRollupError(filename, "FatalError")(e.error); break;
    case "ERROR": onRollupError(filename)(e.error); break;
  }
});
...
exports.onScssError = $.notify.onError( function(e) {
  ...
  let mes = prefix + `${RED}Error${RESET} '${CYAN}${e.plugin}${RESET}': ${e.relativePath} L${e.line}:${e.column}`;
