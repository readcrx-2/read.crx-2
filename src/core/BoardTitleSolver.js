...
const _getBBSMenu = async function() {
  if (_bbsmenu != null) { return _bbsmenu; }
...
const searchFromBBSMenu = async function(url) {
  ...
  const boardName = (left = (left1 = bbsmenu.get(url.href)) != null ? left1 : bbsmenu.get(url2.href)) != null ? left : null;
  return boardName;
};
...
const _formatBoardTitle = function(title, url) {
  switch (url.getTsld()) {
    case "5ch.net": title = title.replace("＠2ch掲示板", ""); break;
    case "2ch.sc": title += "_sc"; break;
    case "open2ch.net": title += "_op"; break;
  }
  return title;
};
...
const searchFromBookmark = function(url) {
  ...
  let left;
  const url2 = url.createProtocolToggled();
  const bookmark = (left = app.bookmark.get(url.href)) != null ? left : app.bookmark.get(url2.href);
  if (bookmark) {
