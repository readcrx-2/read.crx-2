...
`;
  ...
  */
  static _parseNet(text) {
    ...
    if (
      ...
      reg =
        /<div class="post"[^<>]*><div class="number">\d+[^<>]* : <\/div><div class="name"><b>(?:<a [^<>]*href="(?:mailto:|\/cdn-cgi\/l\/email-protection#)([^<>"]*)">|<font [^<>]*>)?(.*?)(?:<\/(?:a|font)>)?<\/b><\/div><div class="date">(.*)<\/div><div class="message"> ?(.*)/;
      separator = "</div></div>";
    } else if (
      ...
      reg =
        /<div class="post"[^<>]*><div class="meta"><span class="number">\d+<\/span><span class="name"><b>(?:<a [^<>]*href="(?:mailto:|\/cdn-cgi\/l\/email-protection#)([^<>"]*)">|<font [^<>]*>)?(.*?)(?:<\/(?:a|font)>)?<\/b><\/span><span class="date">(.*)<\/span><\/div><div class="message">(?:<span class="escaped">)? ?(.*)(?:<\/span>)/;
      separator = "</div></div><br>";
    } else if (text.match(/<footer[^<>]*><br>read\.cgi ver 07\.([6-9]|\d+)/)) {
      titleReg = /<(?:div|h1) id="threadtitle">(.*)\n?<\/(?:div|h1)>/;
      reg =
        /<span class="postid">\d+<\/span><span class="postusername"><b>(?:<a [^<>]*href="(?:mailto:|\/cdn-cgi\/l\/email-protection#)([^<>"]*)">|<font [^<>]*>)?(.*?)(?:<\/(?:a|font)>)?<\/b><\/span>(?:<span style=".*">.*<\/span>)?<\/div>(?:<span style=".*">)?<span class="date">(.*)<\/span><\/div><div class="post-content"> ?(.*)/;
      separator = "</div></div>";
    } else if (text.match(/<footer[^<>]*><br>read\.cgi ver 0(7|8)/)) {
      titleReg = /<(?:div|h1) id="threadtitle">(.*)\n?<\/(?:div|h1)>/;
      reg =
        /<article[^<>]*><details[^<>]*><summary><span class="postid">\d+<\/span><span class="postusername"><b>(?:<a [^<>]*href="(?:mailto:|\/cdn-cgi\/l\/email-protection#)([^<>"]*)">|<font [^<>]*>)?(.*?)(?:<\/(?:a|font)>)?<\/b><\/span>(?:<span style=".*">.*<\/span>)?<\/summary>(?:<span style=".*">)?<span class="date">(.*)<\/span><\/details><section class="post-content"> ?(.*)<\/section>/;
      separator = "</article>";
    } else {
      reg =
        /^(?:<\/?div.*?(?:<br><br>)?)?<dt>\d+.*：(?:<a [^<>]*href="(?:mailto:|\/cdn-cgi\/l\/email-protection#)([^<>"]*)">|<font [^>]*>)?<b>(.*)<\/b>.*：(.*)<dd> ?(.*)<br><br>$/;
      separator = "\n";
    }
    ...
    for (let line of text.split(separator)) {
      ...
      if (regRes) {
        thread.res.push({
          name: regRes[2],
          mail: decodeCloudflareEmail(regRes[1] || ""),
          message: regRes[4],
          other: regRes[3],
        });
      }
  ...
  */
  static _parseJbbsArchive(text) {
    ...
    const reg =
      /<dt[^>]*>\s*\d+ ：\s*(?:<a [^<>]*href="(?:mailto:|\/cdn-cgi\/l\/email-protection#)([^<>"]*)">)?\s*(?:<font [^>]*>)?\s*<b>(.*)<\/b>.*：(.*)\s*<\/dt>\s*<dd>\s*(.*)\s*<br>/;
  ...
  */
  static _parsePink(text, resLength) {
    ...
    if (text.includes('<div class="footer push">read.cgi ver 06')) {
      ...
      reg =
        /^.*?<dl class="post".*><dt class=\"\"><span class="number">(\d+).* : <\/span><span class="name"><b>(?:<a [^<>]*href="(?:mailto:|\/cdn-cgi\/l\/email-protection#)([^<>"]*)">|<font [^>]*>)?(.*?)(?:<\/a>|<\/font>)?<\/b><\/span><span class="date">(.*)<\/span><\/dt><dd class="thread_in"> ?(.*)$/;
      separator = "</dd></dl>";
    } else if (text.includes('<div class="footer push">read.cgi ver 07')) {
      text = text.replace("</h1>", "</h1></div></div><br>");
      reg =
        /<div class="post"[^<>]*><div class="meta"><span class="number">(\d+).*<\/span><span class="name"><b>(?:<a [^<>]*href="(?:mailto:|\/cdn-cgi\/l\/email-protection#)([^<>"]*)">|<font [^<>]*>)?(.*?)(?:<\/(?:a|font)>)?<\/b><\/span>(?:<span style=".*">;.*<\/span>)?(?:<span style=".*">)?<span class="date">(.*)<\/span><\/div><div class="message">(?:<span class="escaped">)? ?(.*)(?:<\/span>)/;
      separator = "</div></div><br>";
    } else if (text.match(/<footer[^<>]*><br>read\.cgi ver 0(8|9)/)) {
      titleReg = /<div id="threadtitle">(.*)\n?<\/div>/;
      reg =
        /<article id="(\d+)"[^<>]*><details[^<>]*><summary><span class="postid">\d+<\/span><span class="postusername"><b>(?:<a [^<>]*href="(?:mailto:|\/cdn-cgi\/l\/email-protection#)([^<>"]*)">|<font [^<>]*>)?(.*?)(?:<\/(?:a|font)>)?<\/b><\/span>(?:<span style=".*">;.*<\/span>)?<\/summary>(?:<span style=".*">)?<span class="date">(.*)<\/span><\/details><section class="post-content"> ?(.*)<\/section>/;
      separator = "</article>";
    } else {
      reg =
        /^(?:<\/?div.*?(?:<br><br>)?)?<dt>(\d+).*：(?:<a [^<>]*href="(?:mailto:|\/cdn-cgi\/l\/email-protection#)([^<>"]*)">|<font [^>]*>)?<b>(.*)<\/b>.*：(.*)<dd> ?(.*)<br><br>$/;
    ...
    for (let line of text.split(separator)) {
      ...
      if (regRes) {
        ...
        thread.res.push({
          name: regRes[3],
          mail: decodeCloudflareEmail(regRes[2] || ""),
          message: regRes[5],
          other: regRes[4],
        });
      }
