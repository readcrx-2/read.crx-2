...
`;
  ...
  */
  static _parseNet(text) {
    ...
    } else if (text.match(/<footer[^<>]*><br>read\.cgi ver 08/)) {
  ...
  */
  static _parsePink(text, resLength) {
    ...
    if (text.includes('<div class="footer push">read.cgi ver 06')) {
      ...
      titleReg = /<div id="threadtitle">(.*)\n?<\/div>/;
      reg =
        /<article id="(\d+)"[^<>]*><details[^<>]*><summary><span class="postid">\d+<\/span><span class="postusername"><b>(?:<a href="mailto:([^<>]*)">|<font [^<>]*>)?(.*?)(?:<\/(?:a|font)>)?<\/b><\/span><\/summary><span class="date">(.*)<\/span><\/details><section class="post-content"> ?(.*)<\/section>/;
      separator = "</article>";
    } else {
      reg =
        /^(?:<\/?div.*?(?:<br><br>)?)?<dt>(\d+).*：(?:<a href="mailto:([^<>]*)">|<font [^>]*>)?<b>(.*)<\/b>.*：(.*)<dd> ?(.*)<br><br>$/;
