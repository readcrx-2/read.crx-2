  ...
  addItem: (items) ->
    ...
    do =>
      ...
      for res in items
        ...
        ignoreResReg = /^ignoreResNumber:(\d+)(-?(\d+))?,(.*)$/
        for ngWord in ngWords
          ...
          # 指定したレス番号はNG除外する
          if ignoreResReg.test(ngWord)
            m = ngWord.match(ignoreResReg)
            if m[3] and m[1] <= resNum and resNum <= m[3]
              continue
            else if Number(m[1]) is resNum
              continue
            else
              ngWord = m[4]
