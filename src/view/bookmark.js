app.boot("/view/bookmark.html", ["Board"], function(Board) {
  ...
  const tableHeaders = ["title", "boardTitle", "res", "unread", "heat", "createdDate"];
  ...
  (function() {
    ...
    let lastSort = (() => { switch (app.config.get("bookmark_sort_save_type")) {
      case "none": return DEFAULT_SORT;
      case "board": return JSON.parse(app.config.get("last_board_sort_config"));
      case "bookmark": return JSON.parse(app.config.get("last_bookmark_sort_config"));
    } })();
  ...
  $view.on("request_reload", function(param) {
    ...
    if ($reloadButton.hasClass("disabled")) { return; }
    ...
    var fn = function(res) {
      ...
      for (board of boardList) {
        ...
        if (loadingServer.has(server)) { continue; }
