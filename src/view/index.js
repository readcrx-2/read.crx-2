...
app.view.Index = class Index extends app.view.View {
  constructor(element) {
    ...
    this.$element.on("tab_removed", async ({target}) => {
      ...
      if (($tmp != null ? $tmp.$(".tab_selected.tab_content") : undefined) != null) {
    ...
    this.$element.on("view_loaded", ({target}) => {
      if (target.matches(".tab_content.iframe_focused")) { return; }
      this.focus(target);
    });
  ...
  */
  focus($iframe, focus) {
    ...
    if (!$iframe.hasClass("iframe_focused")) {
      __guard__(this.$element.C("iframe_focused")[0], x => x.removeClass("iframe_focused"));
      $iframe.addClass("iframe_focused");
    }

    const focusIframe = function($iframe) {
      __guard__($iframe.contentDocument != null ? $iframe.contentDocument.activeElement : undefined, x1 => x1.blur());
      __guard__($iframe.contentDocument != null ? $iframe.contentDocument.getElementsByClassName("content")[0] : undefined, x2 => x2.focus());
    };

    if (focus) {
      if (!$iframe.src.endsWith("empty.html") && (__guard__($iframe.contentDocument != null ? $iframe.contentDocument.getElementsByClassName("content") : undefined, x1 => x1[0]) != null)) {
        focusIframe($iframe);
      } else {
        let fn;
        $iframe.on("load", (fn = function() {
          if ($iframe.src.endsWith("empty.html")) { return; }
          $iframe.off("load", fn);
          focusIframe($iframe);
        })
        );
  ...
  */
  _getLeftFrame($iframe) {
    ...
    if (!$iframe.hasClass("tab_content")) { return null; }
  ...
  */
  focusUp() {
    ...
    if (
      ...
      (__guard__(this.$element.C("iframe_focused")[0].closest(".tab"), x => x.id) === "tab_b")
    ) {
      iframe = this.$element.$("#tab_a iframe.tab_selected");
    }

    if (iframe) { this.focus(iframe); }
  }
  ...
  */
  focusDown() {
    ...
    if (
      $$.I("body").hasClass("pane-3") &&
      (__guard__(this.$element.C("iframe_focused")[0].closest(".tab"), x => x.id) === "tab_a")
    ) {
...
app.boot("/view/index.html", ["BBSMenu"], async function(BBSMenu) {
  ...
  BBSMenu.target.on("change", ({detail: {menu}}) => app.URL.pushServerInfo(menu));
...
app.main = function() {
  ...
  const urlToIframeInfo = function(url, obj) {
    ...
    if (res = /^search:(.+)$/.exec(url)) {
      ...
      const param = {
        query: res[1],
        scheme: (left = obj.scheme != null ? obj.scheme : app.config.get("thread_search_last_mode")) != null ? left : "http"
      };
      return {
  ...
  const iframeSrcToUrl = function(src) {
    ...
    if (res = new RegExp(`^/view/(?:thread|board)\\.html(\\?.+)$`).exec(src)) {
  ...
  (function() {
    ...
    app.bookmark = new app.Bookmark(
      app.config.get("bookmark_id") || "dummy"
    );
  ...
  app.message.on("notify", function({message: text, html, background_color = "#777"}) {
    let func;
    const $div = $__("div");
    ...
    $div.on("click", (func = async function({target, currentTarget: cTarget}) {
      if (!target.matches("a, div:last-child")) { return; }
      $div.off("click", func);
...
`,
  ...
  //更新通知
  browser.runtime.onUpdateAvailable.addListener( async function({version: newVer}) {
...
`,
  ...
  app.message.on("config_updated", function({key, val}) {
    ...
    if (["pane-3", "pane-3h"].includes(val)) {
      for (tmp of tabA.getAll()) {
        ...
        if (!/^https?:/.test(tmpURL)) { continue; }
        if ((new app.URL.URL(tmpURL)).guessType().type !== "thread") { continue; }
  ...
  browser.runtime.onMessage.addListener( function({
    ...
    if (!["written", "written?"].includes(type)) { return; }
  ...
  for (dom of $view.C("tab_tabbar")) {
    dom.on("contextmenu", async function(e) {
      ...
      const $menu = $$.I("template_tab_contextmenu").content.$(".tab_contextmenu").cloneNode(true);
        ...
        switch (false) {
          ...
          case !target.hasClass("close_all"):
            ...
            for (let i = iterable.length - 1; i >= 0; i--) {
              ...
              if (!tab.isLocked(tabid)) { tab.remove(tabid); }
            }
            break;
          //他のタブを全て閉じる
